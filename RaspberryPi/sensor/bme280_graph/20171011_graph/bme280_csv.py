...
bus_number  = 1
i2c_address = 0x76
DATA_DIR_RAS = '/home/pi/data/'     #データ保存用のディレクトリを設定
DATA_DIR_HOST = '/mnt/wind/'     #データ保存用のディレクトリを設定
...
def readData():
	...
	#record data for Ras
	if os.path.exists(DATA_DIR_RAS+record_file_name,):
		writer = csv.writer(open(DATA_DIR_RAS+record_file_name,'ab'))   #アペンドモード
	...
	if os.path.exists(DATA_DIR_HOST+record_file_name,):
		writer = csv.writer(open(DATA_DIR_HOST+record_file_name,'ab'))   #アペンドモード
		writer.writerow([record_time,T,P,H])		              #時刻、温度、気圧、湿度を記録
	else:
		writer = csv.writer(open(DATA_DIR_HOST+record_file_name,'ab'))   #アペンドモード
		writer.writerow(['time','temperature','pressure','humidity'])
		writer.writerow([record_time,T,P,H])		              #時刻、温度、気圧、
